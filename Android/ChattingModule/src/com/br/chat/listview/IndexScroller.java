...
//		float sectionHeight = (mIndexbarRect.height() - 2 * mIndexbarMargin) / (mSections2.length-ContactsSectionIndexer.mSections2.length);
		...
		for (int i = 0; i < mSections.length; i++) {
			/*boolean skip = false;
			for(String ch: ContactsSectionIndexer.mSkipChar){
				if(ch.equals(mSections[i])){
					skip = true;
					break;
				}
			}
