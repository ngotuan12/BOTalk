...
public class SocketManager extends Thread
{
	...
	/*
	 * public void setClient(HashMap<String, ChattingVo> clients,HashMap<String,
	 * Socket> socketlist){ this.clients = clients; this.socketlist =
	 * socketlist; }
	 */
		...
		try
		{
			while (true)
			{
				...
				if (it.hasNext() && it != null)
				{
					...
					socketlist.put(next, vo);
					/*
					 * if(socketlist.get(next).getSocket().isConnected()){
					 * SocketVo vo = socketlist.get(next);
					 * vo.setConnected(true);
					 * //vo.setSocket(socketlist.get((String
					 * )it.next()).getSocket());
					 * 
					 * 
					 * }else{ SocketVo vo = socketlist.get(next);
					 * vo.setConnected(false);
					 * //vo.setSocket(socketlist.get((String
					 * )it.next()).getSocket()); socketlist.replace(next, vo); }
					 */
				}
				else
				{
